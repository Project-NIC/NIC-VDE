...
static bool create_and_format(uint32_t seq) {
  ...
  g_file.sync();
  g_seq = seq;
  /* RAW telemetry → no compression, so keyframe_intv = 0 (N/A). */
  return mla_w_format(&g_w, make_hal(), MLA_SIZE, MLA_CRC_FULL, 12, 0) == MLA_OK;
}

static bool roll_to_next() {              /* current is full → next file */
  g_file.close();
  /* NOTE: for a COMPRESSED (delta/keyframe) stream, the first record of each
   * rolled-over file must be a KEYFRAME so the new file is independently
   * decodable — that is the glue/codec's job (it holds the reference sample).
   * This RAW example needs nothing extra. */
...
void loop() {
   ...
   * subsec = two opaque bytes the glue owns (0 here for slow telemetry; could be
   * a sub-second tick and/or a section/rotation index); compressed = 0 and
   * kf_back = 0 for RAW data. */
  int rc = mla_w_append(&g_w, ts, 0, 1, sample, sizeof(sample), 0, 0);
  if (rc == MLA_E_FULL) {                  /* file full → roll over */
    if (roll_to_next()) {
      Serial.print(F("rolled to ")); Serial.println(g_name);
      mla_w_append(&g_w, ts, 0, 1, sample, sizeof(sample), 0, 0);
    } else {
