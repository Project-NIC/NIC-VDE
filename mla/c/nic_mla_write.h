 ...
 * into the prefix station table (0 = none). subsec = two opaque bytes the glue
 * owns (MLA gives no meaning) — sub-second time and/or section/rotation; use as
 * one u16 or two bytes. compressed = payload is a codec (DMD) blob; kf_back =
 * records back to the owning keyframe (0 = keyframe), 0..127 — compression only.
