...
typedef struct {
    ...
    uint32_t  region_end;        /* log ceiling = file_size − prefix size (mirror above) */
    /* RAM state */
    uint32_t  top_ptr;
    uint32_t  bot_ptr;
...
/* ── Write ──────────────────────────────────────────────────────────────── */
/* station = 1-byte index into the prefix station table (0 = none). subsec = two
 * opaque bytes the glue owns (MLA gives no meaning) — sub-second time and/or
 * section/rotation; use as one u16 or two bytes. compressed = payload is a codec
 * (DMD) blob; kf_back = records back to the owning keyframe (0 = keyframe), 0..127. */
...
typedef struct {
    ...
    uint8_t  has_compressed; uint8_t  compressed;   /* match the compressed flag */
} mla_filter_t;
