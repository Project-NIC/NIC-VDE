"""Tests for the backend abstraction (Entry, Backend defaults)."""
import unittest

from tests import helpers  # noqa: F401  — path bootstrap

from volkov_core.backend import Backend, Entry, Unsupported, BackendError
...
class BackendDefaultTests(unittest.TestCase):
    ...
    def test_mutating_ops_unsupported_by_default(self):
        for call in (
            ...
            lambda: self.b.delete(Entry("x")),
            lambda: self.b.rename(Entry("x"), "y"),
            lambda: self.b.put_file("x", b""),
            lambda: self.b.read(Entry("x")),
        ):
            with self.assertRaises(BackendError):
                call()

    def test_unsupported_is_backend_error(self):
        self.assertTrue(issubclass(Unsupported, BackendError))
